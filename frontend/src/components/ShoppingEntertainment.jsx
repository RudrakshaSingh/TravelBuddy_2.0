...
function Placeholder({ category, className }) {
  ...
  return <div className={`${className} bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center`}><span className="text-3xl">{icons[category] || '🛍️'}</span></div>;
}
      ...
      <div className="bg-zinc-900 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden border border-zinc-800">
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-5">
            ...
            <div className="flex items-center gap-1.5 text-zinc-400"><MapPin className="w-4 h-4 text-pink-500" /><span className="text-sm">{place.distanceKm} km</span></div>
          </div>
          ...
          <a href={`https://www.google.com/maps/dir/?api=1&destination=${place.currentLocation?.lat},${place.currentLocation?.lng}`} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 bg-pink-600 text-white py-3 rounded-xl"><Navigation className="w-5 h-5" />Get Directions</a>
        </div>
      </div>
    </div>
  );
...
function ShoppingEntertainment() {
  ...
  const getUserMarkerIcon = () => window.google ? { path: window.google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: '#ec4899', fillOpacity: 1, strokeColor: '#fff', strokeWeight: 3 } : undefined;
  ...
  if (!isLoaded || loadingLocation) return <div className="min-h-screen flex items-center justify-center bg-black"><Loader2 className="h-16 w-16 animate-spin text-pink-500" /></div>;
    ...
    <div className="min-h-screen bg-black pt-30">
        ...
        <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center space-x-4">
              <div className="bg-gradient-to-br from-pink-600 to-pink-700 p-3 rounded-2xl"><ShoppingBag className="h-7 w-7 text-white" /></div>
              <div><h1 className="text-2xl font-bold text-white">Shopping & Entertainment</h1><p className="text-sm text-zinc-400">{isSearchMode ? `${displayedPlaces.length} results` : `${displayedPlaces.length} nearby`}</p></div>
            </div>
            <button onClick={() => setShowList(!showList)} className="sm:hidden bg-pink-600 text-white p-3 rounded-xl">{showList ? <X className="h-5 w-5" /> : <Filter className="h-5 w-5" />}</button>
          </div>
          <div className="flex gap-3">
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-500" />
              <input type="text" placeholder="Search malls, stores..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} className="w-full pl-12 pr-10 py-3.5 bg-zinc-900 border-2 border-zinc-800 rounded-xl text-zinc-100" />
            ...
            <button onClick={handleSearch} disabled={loadingPlaces} className="px-6 py-3.5 bg-pink-600 text-white font-semibold rounded-xl disabled:opacity-50 flex items-center gap-2">{loadingPlaces ? <Loader2 className="h-5 w-5 animate-spin" /> : <Search className="h-5 w-5" />}<span className="hidden sm:inline">Search</span></button>
          </div>
      ...
      <div className="max-w-[1800px] mx-auto flex flex-col lg:flex-row gap-6 p-4 sm:p-6">
        <div className={`${showList ? 'block' : 'hidden'} lg:block lg:w-96 bg-zinc-900/80 rounded-2xl border border-zinc-800 overflow-hidden`}>
          <div className="bg-zinc-900 border-b border-zinc-800 px-6 py-4"><h2 className="font-bold text-white">{isSearchMode ? '🔍 Search Results' : '📍 Nearby Shops'}</h2></div>
          <div className="overflow-y-auto max-h-[calc(100vh-280px)] p-4 space-y-3">
            {loadingPlaces && <div className="py-12 text-center"><Loader2 className="w-8 h-8 animate-spin text-pink-500 mx-auto" /></div>}
            {!loadingPlaces && displayedPlaces.length === 0 && <p className="text-center text-zinc-400 py-12">No places found</p>}
            {displayedPlaces.map((place) => (
              <div key={place._id} onClick={() => setDetailPlace(place)} className="group bg-zinc-800/50 hover:bg-zinc-800 rounded-xl p-4 cursor-pointer border-2 border-transparent hover:border-pink-500/50">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1 min-w-0"><p className="font-semibold text-white truncate group-hover:text-pink-400">{place.name}</p><div className="flex items-center space-x-2 mt-1"><span className="text-sm text-zinc-300">{place.distanceKm} km</span><Star className="w-3 h-3 text-yellow-500 fill-yellow-500" /><span className="text-xs text-yellow-500">{place.rating}</span></div></div>
                  <ChevronRight className="h-5 w-5 text-zinc-600 group-hover:text-pink-500 self-center" />
                </div>
              </div>
            ))}
        ...
        <div className="flex-1 min-h-[500px]">
          <div className="bg-zinc-900/80 rounded-2xl h-full border border-zinc-800 overflow-hidden">
            <div className="bg-zinc-900 p-5 border-b border-zinc-800 flex items-center space-x-3"><Navigation className="h-5 w-5 text-pink-400" /><span className="font-bold text-white">Map View</span></div>
            <div className="h-[calc(100%-80px)] min-h-[450px] relative">
              <GoogleMap mapContainerStyle={containerStyle} center={selectedPlace?.currentLocation || userLocation || DEFAULT_CENTER} zoom={userLocation ? 12 : 5} options={{ styles: darkMapStyles, streetViewControl: false, mapTypeControl: false }}>
                {userLocation && <><Marker position={userLocation} icon={getUserMarkerIcon()} /><Circle center={userLocation} radius={DEFAULT_RADIUS} options={{ fillColor: '#ec489933', strokeColor: '#ec4899' }} /></>}
              ...
              {selectedPlace && (
                <div className="absolute bottom-4 left-4 right-4 lg:max-w-sm bg-zinc-900/95 rounded-2xl border border-zinc-800 p-5">
                  ...
                  <button onClick={() => setDetailPlace(selectedPlace)} className="w-full py-2 bg-pink-600/20 text-pink-400 rounded-lg border border-pink-500/30">View Details</button>
                </div>
              )}
            </div>
          </div>
