...
function PlaceDetailModal({ place, onClose }) {
  ...
  const getCategoryColor = (category) => {
    switch (category) {
      case 'Mall': return 'bg-pink-500/20 text-pink-400 border-pink-500/30';
      case 'Supermarket': return 'bg-green-500/20 text-green-400 border-green-500/30';
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-16rem)]">
          ...
          {place.phoneNumber && (
            <div className="mb-5">
              ...
              <a href={`tel:${place.phoneNumber}`} className="flex items-center gap-2 text-pink-400">
          ...
          <div className="mt-6 pt-4 border-t border-zinc-800">
            <a href={`https://www.google.com/maps/dir/?api=1&destination=${place.currentLocation?.lat},${place.currentLocation?.lng}`} target="_blank" rel="noopener noreferrer"
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-pink-600 to-pink-700 text-white font-semibold py-3 px-6 rounded-xl">
...
function ShoppingEntertainment() {
  ...
  const fetchNearbyPlaces = useCallback(async (lat, lng, radius) => {
    ...
    setError('');
    try {
      const response = await placesService.getNearbyShopping(lat, lng, radius);
      if (response.statusCode === 200) setNearbyPlaces(response.data || []);
      else setError(response.message || 'Failed to fetch');
    } catch (err) {
      console.error('Error:', err);
      setError('Failed to fetch nearby shopping.');
    } finally {
  ...
  if (!isLoaded || loadingLocation) {
    return (<div className="min-h-screen flex items-center justify-center bg-black"><Loader2 className="h-16 w-16 animate-spin text-pink-500" /></div>);
  }
    ...
    <div className="min-h-screen bg-black pt-30">
        ...
        <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center space-x-4">
              <div className="relative bg-gradient-to-br from-pink-600 to-pink-700 p-3 rounded-2xl shadow-lg">
                <ShoppingBag className="h-7 w-7 text-white" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-white">Shopping & Entertainment</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowList(!showList)} className="sm:hidden bg-pink-600 text-white p-3 rounded-xl">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search malls, stores..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                className="w-full pl-12 pr-4 py-3.5 bg-zinc-900 border-2 border-zinc-800 rounded-xl focus:ring-2 focus:ring-pink-500 text-zinc-100" />
            </div>
            <button onClick={handleSearch} disabled={loadingPlaces} className="px-6 py-3.5 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-xl disabled:opacity-50 flex items-center gap-2">
      ...
      <div className="max-w-[1800px] mx-auto flex flex-col lg:flex-row gap-6 p-4 sm:p-6">
        <div className={`${showList ? 'block' : 'hidden'} lg:block lg:w-96 bg-zinc-900/80 rounded-2xl border border-zinc-800 overflow-hidden`}>
          <div className="bg-zinc-900 border-b border-zinc-800 px-6 py-4"><h2 className="font-bold text-white">Shopping Spots</h2></div>
          <div className="overflow-y-auto max-h-[calc(100vh-280px)] p-4 space-y-3">
            {loadingPlaces && <Loader2 className="w-8 h-8 animate-spin text-pink-500 mx-auto" />}
              ...
              <div key={place._id} onClick={() => setDetailPlace(place)} className="group bg-zinc-800/50 hover:bg-zinc-800 rounded-xl p-4 cursor-pointer border-2 border-transparent hover:border-pink-500/50">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-white group-hover:text-pink-400">{place.name}</p>
                  ...
                  <ChevronRight className="h-5 w-5 text-zinc-600 group-hover:text-pink-500" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-zinc-900/80 rounded-2xl h-full border border-zinc-800 overflow-hidden">
            <div className="bg-zinc-900 p-5 border-b border-zinc-800 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <Navigation className="h-5 w-5 text-pink-400" />
            ...
            <div className="h-[calc(100%-80px)] min-h-[450px] relative">
              <GoogleMap mapContainerStyle={containerStyle} center={selectedPlace?.currentLocation || userLocation || DEFAULT_CENTER} zoom={userLocation ? 11 : 5} options={{ styles: darkMapStyles, streetViewControl: false, mapTypeControl: false }}>
                {userLocation && (<><Marker position={userLocation} icon={getUserMarkerIcon()} /><Circle center={userLocation} radius={radiusMeters} options={{ fillColor: '#ec489933', strokeColor: '#ec4899' }} /></>)}
